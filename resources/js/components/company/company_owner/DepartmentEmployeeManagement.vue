    ...
    <div>
        ...
        >
                        ...
                        <tr v-for="(employee,index) in selectedDepartment.user">
                            <td class="table-width-60">
                                <i class="fa fa-minus-circle 2x deleteEmployee" v-if="MultiDeleteToggle == false"
                                   @click="initDetachSingleEmployeeModal(employee)"></i>
                                <label class="form-checkbox" v-else>
                                    <input type="checkbox" class="form-checkbox disabled" :value="employee.id"
                                           v-model="selectedIds">
                                    <i class="form-icon"></i>
                                </label>
                            </td>
            ...
            <template v-slot:footer>
                <div class="text-right">
                    <b-button v-if="MultiDeleteToggle != false" variant="danger" size="sm"
                              @click="openConfirmDeleteModal">confirm detachment
                    </b-button>
                    <b-button v-else size="sm" variant="primary" @click="initAssignEmployeeModal">attach employees to
                        department
                    </b-button>
                </div>
            </template>
                    ...
                    <b-row>
                        <b-col>
                            <div class="text-center">
                                <b-row>
                                    <b-col>
                                        <h4 v-if="this.selected.length > 1">You have selected {{this.selected.length}}
                                            employees to detach from department.</h4>
                                        <h4 v-else>You have selected {{this.selected.length}} employee to detach from
                                            department.</h4>
                                    </b-col>
                                </b-row>
                                <b-row>
                                ...
                                <b-row>
                                    <b-col>
                                        <small><p v-b-toggle.collapse-employee-list class="text-center make-fake-link">
                                            Show/hide selected employees.</p></small>
                                    </b-col>
                                </b-row>
                            </div>
                        </b-col>
                                ...
                                <b-collapse id="collapse-employee-list" v-model="employeeDeletionListCollapse">
                                    ...
                                    <table class="table table-borderless">
                                        ...
                                        <tr v-for="(employee,index) in this.selected">
                                            <td>{{employee.name}}</td>
                                            <td class="text-right">
                                                <b-button v-if="selected.length > 1" size="sm" variant="success"
                                                          @click="removeFromMassDeletionList(employee.id,index)">remove
                                                    from list
                                                </b-button>
                                                <b-button v-else size="sm" variant="success"
                                                          @click="removeFromMassDeletionList(employee.id,index)"
        ...
        <b-modal v-model="assignEmployeeModal" v-if="this.selectedDepartment !== null" size="lg">
            <template v-slot:modal-title>
                <div class="text-left">
                    <i class="fa fa-question-circle" id="attach-employees"></i>
                    <b-popover target="attach-employees" triggers="hover" placement="top">
                        <template v-slot:title>Attach employees</template>
                        <!--                                TODO::Write information for this popover.-->
                        Info TBD
                    </b-popover>
                </div>
            </template>
            <b-row>
                <b-col align-self="center">
                    <div class="text-center">
                        <h4>Attach employees to <i>{{this.selectedDepartment.name}}</i></h4>
                    </div>
                </b-col>
            </b-row>
            <b-row>
        ...
        <b-modal v-model="detachSingleEmployeeModal" v-if="this.selectedDepartment && this.employee !== null" size="md">
            ...
            <b-row>
                <b-col>
                    <p>You're trying to detach <strong><i>{{this.employee.name}}</i></strong> from the department. Are
                        you sure?</p>
                </b-col>
            </b-row>
            <template v-slot:modal-footer>
                <b-button variant="primary" size="sm" @click="cancelDetachSingleEmployee">cancel</b-button>
                <b-button variant="success" size="sm" @click="confirmDetachSingleEmployee">detach</b-button>
            </template>
        </b-modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initDetachSingleEmployeeModal(employee) {
                this.detachSingleEmployeeModal = !this.detachSingleEmployeeModal;
            ...
            filterEmployeesAlreadyInDepartment() {
                for (let s in this.selectedEmployees) {
                    for (let n in this.selectedDepartment.user) {
                        ...
                        } else if(this.selectedEmployees[s].id == this.selectedDepartment.user[n].id){
            ...
            updateEmployees() {
                this.employeeCount = this.selectedDepartment.user.length;
                this.$root.$emit('updateDepartmentEmployees', this.employeeCount, this.selectedDepartment.id,this.selectedDepartment.user);
                ...
                if (this.selectedEmployees.length == 0) {
                    ...
                    axios.post(url, {
                        ...
                        for (let x in this.selectedEmployees) {
                            //Adding new users to department.
